...
export function CustomersContent() {
  ...
  async function toggleActive(r) {
    ...
    if (goingInactive && !confirm(`Deactivate ${r.company}? They'll disappear from order/allocation pickers, but all their existing orders and history stay exactly as-is. You can reactivate anytime.`)) return;
    const { error } = await supabase.from('customers').update({ active: !goingInactive }).eq('customer_id', r.customer_id);
  ...
  return (
    <>
      ...
      {rows.filter(r => showInactive || r.active !== false).map(r => {
        ...
        return (
          <div key={r.customer_id} style={{ ...card, opacity: r.active === false ? 0.6 : 1 }}>
